...
function getWebcam()
{
    ...
    // Query MongoDB for webcam snapshots filtered by student_id, session_id, type, and content
    $filter = [
        //'StudentId' => (int) $student_id, // Assuming student_id is an integer
        //'SessionId' => (int) $session_id, // Assuming session_id is an integer
        'type' => 'camera image' // Filter by type containing 'camera image'
    ];
    ...
    foreach ($rows as $row) {
        $imageData = $row->content; // Assuming 'content' contains the base64 image data
        $timestampUTC = $row->timestamp->toDateTime()->setTimezone(new DateTimeZone('UTC'));
        $timestampGMT8 = $timestampUTC->setTimezone(new DateTimeZone('Asia/Singapore')); // Adjust timezone as per your GMT+8 location
        $formattedDateTime = $timestampGMT8->format('Y-m-d H:i:s');

        // Generate carousel item HTML
        $carouselItems .= '<div class="carousel-item ' . $activeClass . '">';
        $carouselItems .= '<img src="data:image/png;base64,' . $imageData . '" class="d-block w-100 carousel-img" alt="Snapshot" data-bs-toggle="modal" data-bs-target="#modalWebcam' . $slideIndex . '">';
        ...
        $carouselItems .= '<p>Captured at: ' . $formattedDateTime . ' GMT+8</p>'; // Display formatted timestamp
    ...
    foreach ($rows as $row) {
        ...
        $timestamp = $row->timestamp->toDateTime()->format('Y-m-d H:i:s'); // Format timestamp field
        ...
        echo '<h2>Captured at: ' . $timestamp . '</h2>'; // Display formatted timestamp
        echo '</div>';
        echo '</div>';
        echo '</div>';
        echo '</div>';
