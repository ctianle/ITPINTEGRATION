...
if ($uploadOk == 0) {
    ...
    $error = "\n" . $date_time . " Error uploading: File Invalid " . htmlspecialchars( basename( $_FILES["fileToUpload"]["name"]));
    // error_log(print_r($error, true), 3, $_SERVER['DOCUMENT_ROOT'] . "/system_error.log");
    
    $logfilelocation = "/var/logs/myapp/system_error.log";
    error_log(print_r($error, true), 3, $logfilelocation);
